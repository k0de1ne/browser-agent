...
import {
  ...
  MAX_MAIN_CONTENT_LENGTH,
  MAX_HEADINGS_TO_DISPLAY,
...
export class DOMExtractor {
  public async extractSimplifiedDOM(page: Page, filter: ContentFilter = {}): Promise<SimplifiedElement[]> {
    ...
    const elements = await page.evaluate((filterParam) => {
      ...
      const extractElementData = (el: Element, idCounter: number): SimplifiedElement => {
        ...
        ['href', 'type', 'placeholder', 'value', 'aria-label', 'title', 'name', 'id', 'class', 'role', 'data-testid'].forEach(attr => {
