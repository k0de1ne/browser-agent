...
export class AgentVisualizer {
  ...
  public displayPlan(): void {
    ...
    console.log('\n' + chalk.bgBlue.white.bold(' 📋 TASK PLAN '));
    ...
    this.currentPlan.steps.forEach((step, index) => {
      ...
      if (step.result) {
        console.log(
          chalk.gray(`   └─ ${step.result.substring(0, 80)}${step.result.length > 80 ? '...' : ''}`)
        );
      }
    });
  ...
  public displayThinking(thinking: string, stream = false): void {
    if (stream) {
      if (this.thinkingBuffer.length === 0) {
        console.log('\n' + chalk.bgMagenta.white.bold(' 🧠 AGENT THINKING '));
        console.log(chalk.gray('─'.repeat(60)));
      }
  ...
  public displayReasoning(reasoning: string): void {
    console.log('\n' + chalk.bgCyan.black.bold(' 💡 REASONING ') + '\n');
  ...
  public displayActionHistory(lastN = 10): void {
    ...
    console.log('\n' + chalk.bgGreen.black.bold(' 📜 ACTION HISTORY ') + '\n');
    ...
    recentActions.forEach((action, index) => {
      ...
      console.log(
        chalk.gray(`   📥 Arguments: ${JSON.stringify(action.args).substring(0, 60)}...`)
      );
      console.log(
        chalk.gray(
          `   📤 Result: ${action.result.substring(0, 80)}${action.result.length > 80 ? '...' : ''}`
        )
      );
  ...
  public displayCurrentStep(iteration: number, tool: string, args: Record<string, unknown>): void {
    console.log('\n' + chalk.bgYellow.black.bold(` ⚙️  STEP ${iteration} `) + '\n');
    console.log(chalk.yellow(`🔧 Tool: ${chalk.bold(tool)}`));
  ...
  public displayStepResult(result: string, status: 'success' | 'failed'): void {
    ...
    const label = status === 'success' ? '✓ Result' : '✗ Error';
  ...
  public displayTaskCompletion(success: boolean, summary: string): void {
    if (success) {
      console.log('\n' + chalk.bgGreen.white.bold(' ✅ TASK COMPLETED '));
      console.log(chalk.green('\n📋 Summary:'));
      console.log(chalk.white(`   ${summary}`));
    } else {
      console.log('\n' + chalk.bgRed.white.bold(' ❌ TASK NOT COMPLETED '));
