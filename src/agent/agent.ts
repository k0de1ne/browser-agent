  ...
  async executeTask(task: string): Promise<void> {
    ...
    while (!this.isTaskComplete && iteration < this.config.maxIterations!) {
      ...
      try {
        ...
        logger.info(chalk.gray(`   📊 Tokens: prompt=${response.usage?.prompt_tokens}, completion=${response.usage?.completion_tokens}, total=${response.usage?.total_tokens}`));
        ...
        if (message.tool_calls && message.tool_calls.length > 0) {
          ...
          logger.warn(chalk.yellow('⚠️  No tool calls received despite using tool_choice: required'));
  ...
  private async executeTool(toolName: string, args: any): Promise<any> {
    ...
    switch (toolName) {
      ...
      case 'get_page_content': {
        ...
        const elementsStr = this.domExtractor.formatElementsForLLM(this.currentElements, filter.maxElements || 50);
      ...
      case 'find_element': {
        const elements = await page.evaluate(({ selector, text, attribute, attribute_value }) => {
          let candidates: Element[] = [];
          if (selector) {
            candidates = Array.from(document.querySelectorAll(selector));
          } else {
            candidates = Array.from(document.querySelectorAll('a, button, input, textarea, select, [role="button"], [onclick]'));
          }
          ...
          if (attribute && attribute_value) {
            candidates = candidates.filter(el => {
              ...
              return attrVal && attrVal.includes(attribute_value);
            });
          }
          ...
          if (candidates.length > 0) {
            ...
            return {
              ...
              text: el.innerText?.trim().substring(0, 100) || (el as HTMLInputElement).value || '',
              attributes,
        ...
        }, { selector: args.selector, text: args.text, attribute: args.attribute, attribute_value: args.attribute_value });
      ...
      case 'read_page_text': {
        try {
          const text = await page.evaluate(({ selector, maxLength }) => {
