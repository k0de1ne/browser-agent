...
export class SecurityLayer {
  ...
  public static assessAction(context: DestructiveActionContext): SecurityAssessment {
    ...
    if (context.elementAttributes) {
      const attrAssessment = this.checkAttributesForDestructiveIndicators(context.elementAttributes);
      if (attrAssessment) {
  ...
  private static checkAttributesForDestructiveIndicators(attributes: Record<string, string>): SecurityAssessment | null {
    const attrString = JSON.stringify(attributes).toLowerCase();

    if (attributes.type === 'submit' || attributes.role === 'button') {
      const indicators = ['delete', 'remove', 'buy', 'purchase', 'pay', 'submit-payment', 'checkout', 'confirm-order'];
  ...
  private static assessTypeAction(context: DestructiveActionContext): SecurityAssessment | null {
    ...
    if (attrString.includes('card') || attrString.includes('cvv') || attrString.includes('credit')) {
      return {
        isDestructive: true,
        riskLevel: 'critical',
  ...
  public static generateConfirmationMessage(assessment: SecurityAssessment, context: DestructiveActionContext): string {
